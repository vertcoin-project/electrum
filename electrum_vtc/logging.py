...
def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_vtc_log_*.log")), reverse=True)
    for f in files[keep:]:
        try:
            os.remove(str(f))
        except OSError as e:
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_vtc_log_{timestamp}_{PID}.log"
...
def configure_logging(config):
    ...
    if is_android:
        ...
        build_config = autoclass("org.electrum_vtc.electrum_vtc.BuildConfig")
